...
## 余談
1. FD-ARK製作の経緯
FDX68シールドという作品がBOOTHで売られていて[^3]、フロッピーディスクを読みだすこともFDDエミュレータになるらしいという漠然とした知識だけありました。具体的な使い方や仕組みは判りませんでした。ふとゲーム保存協会のツイート[^4]が目にとまり、国立国会図書館@NDLJPフロッピーディスク長期保存対策に関する調査報告書というの知ってななめ読みしてみました。これには様々な理由でフロッピーディスクが劣化して使用できなくなってしまうと書かれており、所持しているMSXに関するフロッピーディスクは30年を超えている物が多く、結果これらフロッピーディスクの危機感を募らせてしまうことになりました。
BOOTHで販売されていることで一番身近に感じるFDX68シールドを手に入れたい気持ちがあったのですがなかなか購入できません。調査報告書の中ではツールのひとつにKryoFluxがh紹介されていて公式ページのリンクから購入可能と書かれていたので購入してみることにしました。で、ちゃんと調べてから購入すれば良かったのですが、FDX68シールドの前知識でKryoFluxにもFDDエミュレータの機能があると思い込んでしまっていたんですね。ただ、発注後に取説書読んでいてそれに気が付いたわけですが、同時にFDDって意外とシンプルな仕組みなんだとその時に知りました。これならKryoFluxで読み込んだデータを使用するFDDエミュレータを自分でも作れるだろう、いや作ってしまえと思うに至りました。ただただ、しっかり作ろうとすると細部が大変になりますよね。フロッピーディスクのイメージ化機能もいっしょに作れば、データフォーマットも独自定義できるしもっと楽に作れるとおもい、結果、今回のFD-ARKになりました。KryoFluxはKryoFluxで、FD-ARKが失敗した時のためにフロッピーディスクのイメージ化に使用しています。また、FD-ARKで読み出して、そして書き出した後に、正しく読み書きできているかをKryoFluxでチェックするなんてことに使用しています。
